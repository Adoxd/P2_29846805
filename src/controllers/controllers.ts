...
class ContactsController {
  ...
  async add(req: Request, res: Response): Promise<void> {
    ...
    `http://api.ipstack.com/${ip}?access_key=${process.env.KEYIPAPI}`
      ...
      const recipients = ['programacion2ais@yopmail.com', 'tesoreroadolfo23@gmail.com'];
  ...
  async index(req: Request, res: Response): Promise<void> {
    try {
      res.render('index', {
        ...
        isAdmin: false,
        title: 'TechFix',
        description: 'programacion 2 task 4 techfix page',
        imageUrl: 'https://p2-29846805-4.onrender.com/img/261.svg',
        pageUrl: 'https://p2-29846805-4.onrender.com'
      });
    } catch (error: any) {
      console.error(error.message);
